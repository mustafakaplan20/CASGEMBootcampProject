...
public class EmployeeManager implements EmployeeService{
	...
	public DataResult<AddEmployeeResponse> add(AddEmployeeRequest addEmployeeRequest) {
		...
		checkIfEmployeeExistsByNationalId(employee.getNationalId());
	...
	public DataResult<UpdateEmployeeResponse> update(UpdateEmployeeRequest updateEmployeeRequest) {
		...
		checkIfEmployeeExistsById(employee.getId());
		employeeRepository.save(employee);
	...
	private void checkIfEmployeeExistsByNationalId(String nationalId){
		...
		if(employee!=null||nationalId.length()==11) throw new BusinessException(Messages.EmployeeExists);

	}
	private void checkIfEmployeeExistsById(int id){
		Employee employee=employeeRepository.findById(id).orElseThrow(()-> new BusinessException(Messages.EmployeeNotExist));
	}
}
